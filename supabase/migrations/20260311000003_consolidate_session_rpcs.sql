-- Consolidate session RPCs that were defined across 3 separate migrations:
--   1. add_match_sessions.sql (original get_discoverable_sessions)
--   2. fix_discoverable_sessions_rpc.sql (fixed text[] vs jsonb comparison)
--   3. auto_end_expired_sessions.sql (added auto-cleanup + redefined get_discoverable_sessions)
--
-- This migration ensures both functions exist with their final correct versions
-- and that get_discoverable_sessions always calls auto_end_expired_sessions first.

-- 1. Auto-end expired sessions function
...
-- 2. Get discoverable sessions — calls auto_end first, uses correct text[] comparison
